...
namespace Shops.UI
{
    public class ShopsUIMultiSelectionMenu<T> : BaseShopsUIMenu<T>
    {
        ...
        public ShopsUIMultiSelectionMenu(IShopUIManager manager, string title = "")
            : base(manager)
        {
            Title = title;
        ...
        public override void Show()
        {
            ...
            List<MenuAction<T>> actions = AnsiConsole.Prompt(new MultiSelectionPrompt<MenuAction<T>>()
                .Title(Title)
                .AddChoices(_selectionActions.ToArray()));
            foreach (MenuAction<T> action in actions)
            {
                action.Invoke();
            }
